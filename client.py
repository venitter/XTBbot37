...
def main():
    ...
    # sclient.subscribeCandles("BITCOIN")

    while True:
        def can_i_open():
            resp = client.commandExecute('getTrades', dict(
                openedOnly=True
            ))
            return resp['returnData']

        def current_price(typ):
            timenow = int(time.time())
            resp = client.commandExecute('getTickPrices', dict(
                level=0,
                symbols='OIL',
                timestamp=timenow
            ))
            r = 0
            if typ == 'ASK':
                r = resp['returnData']['quotations'][0]['ask']
            if typ == 'BID':
                r = resp['returnData']['quotations'][0]['bid']
            return r

        # jezeli zwraca 1 to trend na BUY, jezeli zwraca -1 to trend na SELL
        def trend_check():
            def trend_input(h):
                timenow = int(time.time()) * 1000
                starttime = timenow - h * 60 * 60 * 1000
                endtime = timenow
                resp = client.commandExecute("getChartRangeRequest", dict(info=dict(
                    start=starttime,
                    end=endtime,
                    period=1,
                    symbol="OIL")))
                ...
                df.values.tolist()
                return df.iloc[-1]['open'] - df.iloc[0]['open']

            trend_1h = trend_input(0.5)
            trend_24h = trend_input(24)

            diff1h = 10
            diff24h = 50

            #if trend_1h or trend_24h is False:
            #    print("Market is closed")
            #    return 0
            if trend_1h > diff1h and trend_24h > diff24h:
                print("1H GOING UP: " + str(trend_1h) + " / 24H GOING UP: " + str(trend_24h))
                return 0
            elif trend_1h < -diff1h and trend_24h > diff24h:
                print("1H GOING DOWN: " + str(trend_1h) + " / 24H GOING UP: " + str(trend_24h))
                return 1
            elif trend_1h > diff1h and trend_24h < -diff24h:
                print("1H GOING UP: " + str(trend_1h) + " / 24H GOING DOWN: " + str(trend_24h))
                return -1
            elif trend_1h < -diff1h and trend_24h < -diff24h:
                print("1H GOING DOWN: " + str(trend_1h) + " / 24H GOING DOWN: " + str(trend_24h))
                return 0
            else:
                print("1H GOING STALL: " + str(trend_1h) + " / 24H GOING STALL: " + str(trend_24h))

        current_price_ask = current_price('ASK')
        current_price_bid = current_price('BID')

        def buy_order():
            resp = client.commandExecute("tradeTransaction", dict(tradeTransInfo=dict(
                cmd=0,
                customComment="BUY",
                price=current_price_bid,
                tp=current_price_bid + 0.2,
                sl=current_price_bid - 0.6,
                symbol="OIL",
                type=0,
                volume=0.01)))
        ...
        def sell_order():
            resp = client.commandExecute("tradeTransaction", dict(tradeTransInfo=dict(
                cmd=1,
                customComment="SELL",
                price=current_price_ask,
                tp=current_price_ask - 0.2,
                sl=current_price_ask + 0.6,
                symbol="OIL",
                type=0,
                volume=0.01)))
